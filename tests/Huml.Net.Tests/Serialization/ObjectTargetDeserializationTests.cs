...
public class ObjectTargetDeserializationTests
{
    ...
    {
        var map = Huml.Deserialize<Dictionary<string, object?>>(
    ...
    {
        var map = Huml.Deserialize<Dictionary<string, object?>>(input, HumlOptions.LatestSupported);
    ...
    public void Object_dictionary_round_trips_through_serialiser()
    {
        ...
        var emitted = Huml.Serialize(map);
        var restored = Huml.Deserialize<Dictionary<string, object?>>(emitted, HumlOptions.Default);
    ...
    {
        var act = () => Huml.Deserialize<Dictionary<string, object?>>("123", HumlOptions.LatestSupported);
    ...
    {
        var value = Huml.Deserialize<object>("123", HumlOptions.LatestSupported);
    ...
    {
        Huml.Deserialize<long>("123", HumlOptions.LatestSupported).Should().Be(123L);
        Huml.Deserialize<string>("\"hi\"", HumlOptions.LatestSupported).Should().Be("hi");
    }

    [Fact]
    public void Root_sequence_into_typed_list_unwraps()
    {
        var list = Huml.Deserialize<List<long>>("1, 2, 3", HumlOptions.LatestSupported);
